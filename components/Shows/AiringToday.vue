  ...
  <div>
    <v-row class="d-flex justify-center">
      ...
      <v-pagination
        v-model="currentPage"
        :total-visible="7"
        :length="airingTodayShows.totalPages"
        @input="updatePage"
      />
    </v-row>
...
export default {
  data () {
    return {
      mediaType: 'show',
      componentId: 'airingTodayShows',
      airingTodayUrl: `https://api.themoviedb.org/3/tv/airing_today?api_key=${process.env.API_SECRET}&language=en-US`,
      currentPage: 1,
      nextPage: 0,
      totalPages: 1,
      results: []
  ...
  methods: {
    loadAiringToday () {
      const URL = `${this.airingTodayUrl}&page=${this.currentPage}`
    ...
    updatePage () {
      if (this.currentPage === this.nextPage) {
        ...
        this.opts = cfg.renderObject(`${this.airingTodayUrl}&page=${this.currentPage}`, this.componentId)
        this.loadAiringToday()
      }
    }
  }

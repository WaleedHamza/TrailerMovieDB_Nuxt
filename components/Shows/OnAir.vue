...
export default {
  data () {
    return {
      mediaType: 'show',
      componentId: 'onAirShows',
      onAirUrl: `https://api.themoviedb.org/3/tv/on_the_air?api_key=${process.env.API_SECRET}&language=en-US`,
      currentPage: 1,
      nextPage: 0,
      totalPages: 1,
      results: []
  ...
  methods: {
    loadOnAir () {
      const URL = `${this.onAirUrl}&page=${this.currentPage}`
    ...
    updatePage () {
      if (this.currentPage === this.nextPage) {
        ...
        this.opts = cfg.renderObject(`${this.onAirUrl}&page=${this.currentPage}`, this.componentId)
        this.loadOnAir()
      }
    }
  }
